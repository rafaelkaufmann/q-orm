...
var P		= require('bluebird'),
	connect	= P.promisify(require('orm').connect),
...
function setupConnectionMethods(connection) {
	...
	connection.qExecQuery = P.promisify(connection.driver.execQuery, connection.driver);
...
function defineModel(name, properties, opts) {
	...
	Model.qCreate = function () {
		return Model.createAsync.apply(Model.createAsync, Array.prototype.slice.apply(arguments))
		.then(extendInstance);
	};

	Model.qGet = function () {
		return Model.getAsync.apply(Model.createAsync, Array.prototype.slice.apply(arguments))
		.then(extendInstance);
	};

	Model.qOne = function () {
		return Model.oneAsync.apply(Model.createAsync, Array.prototype.slice.apply(arguments))
		.then(extendInstance);
	};

	Model.qAll = function () {
		return Model.allAsync.apply(Model.createAsync, Array.prototype.slice.apply(arguments))
		.then(extendInstance);
	};
...
function extendInstanceWithAssociation(Instance, association) {
	...
	var hasAccessor = P.promisify(Instance[association.hasAccessor], Instance),
		...
		delAccessor = P.promisify(Instance[association.delAccessor], Instance),
		addAccessor = P.promisify(Instance[association.delAccessor], Instance);
